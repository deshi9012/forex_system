describe 'Morris.Bar', ->
  describe 'when using vertical grid', ->
    ...
    describe 'svg structure', ->
      it 'should contain extra rectangles for vertical grid', ->
        ...
        $('#graph').find("rect").length.should.equal 6

    describe 'svg attributes', ->
      it 'should have to bars with verticalGrid.color', ->
        chart = Morris.Bar $.extend {}, defaults
        $('#graph').find("rect[fill='#{defaults.verticalGridColor}']").length.should.equal 2
      it 'should have to bars with verticalGrid.color', ->
        chart = Morris.Bar $.extend {}, defaults
        $('#graph').find("rect[fill-opacity='#{defaults.verticalGridOpacity}']").length.should.equal 2
  ...
  describe 'svg structure', ->
    ...
    it 'should contain a rect for each bar', ->
      chart = Morris.Bar $.extend {}, defaults
      $('#graph').find("rect").length.should.equal 4

    it 'should contain 5 grid lines', ->
      chart = Morris.Bar $.extend {}, defaults
      $('#graph').find("path").length.should.equal 5

    it 'should contain 7 text elements', ->
      chart = Morris.Bar $.extend {}, defaults
      $('#graph').find("text").length.should.equal 7
  ...
  describe 'svg attributes', ->
    ...
    it 'should have a bar with the first default color', ->
      chart = Morris.Bar $.extend {}, defaults
      $('#graph').find("rect[fill='#0b62a4']").length.should.equal 2

    it 'should have a bar with no stroke', ->
      chart = Morris.Bar $.extend {}, defaults
      $('#graph').find("rect[stroke='none']").length.should.equal 4

    it 'should have text with configured fill color', ->
      chart = Morris.Bar $.extend {}, defaults
      $('#graph').find("text[fill='#888888']").length.should.equal 7

    it 'should have text with configured font size', ->
      chart = Morris.Bar $.extend {}, defaults
      $('#graph').find("text[font-size='12px']").length.should.equal 7
  ...
  describe 'when setting bar radius', ->
    describe 'svg structure', ->
      ...
      it 'should contain a path for each bar', ->
        chart = Morris.Bar $.extend {}, defaults
        $('#graph').find("path").length.should.equal 9
  ...
  describe 'barSize option', ->
    describe 'svg attributes', ->
      ...
      it 'should set width to @options.barSize if possible', ->
        chart = Morris.Bar $.extend {}, defaults
        $('#graph').find("rect[width='#{defaults.barSize}']").length.should.equal 11
