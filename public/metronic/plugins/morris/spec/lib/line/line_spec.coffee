describe 'Morris.Line', ->
  ...
  describe 'svg structure', ->
    ...
    it 'should contain a circle for each data point', ->
      chart = Morris.Line $.extend {}, defaults
      $('#graph').find("circle").length.should.equal 2

    it 'should contain 5 grid lines', ->
      chart = Morris.Line $.extend {}, defaults
      $('#graph').find("path[stroke='#aaaaaa']").length.should.equal 5

    it 'should contain 9 text elements', ->
      chart = Morris.Line $.extend {}, defaults
      $('#graph').find("text").length.should.equal 9
  ...
  describe 'svg attributes', ->
    ...
    it 'should have circles with configured fill color', ->
      chart = Morris.Line $.extend {}, defaults
      $('#graph').find("circle[fill='#0b62a4']").length.should.equal 2

    it 'should have circles with configured stroke width', ->
      chart = Morris.Line $.extend {}, defaults
      $('#graph').find("circle[stroke-width='5']").length.should.equal 2

    it 'should have circles with configured stroke color', ->
      chart = Morris.Line $.extend {}, defaults
      $('#graph').find("circle[stroke='#ffffff']").length.should.equal 2

    it 'should have line with configured line width', ->
      chart = Morris.Line $.extend {}, defaults
      $('#graph').find("path[stroke-width='3']").length.should.equal 1

    it 'should have text with configured font size', ->
      chart = Morris.Line $.extend {}, defaults
      $('#graph').find("text[font-size='12px']").length.should.equal 9

    it 'should have text with configured font size', ->
      chart = Morris.Line $.extend {}, defaults
      $('#graph').find("text[fill='#888888']").length.should.equal 9

    it 'should have circle with configured size', ->
      chart = Morris.Line $.extend {}, defaults
      $('#graph').find("circle[r='5']").length.should.equal 2
