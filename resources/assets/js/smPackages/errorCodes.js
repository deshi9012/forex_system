 ...
 * @property VARIANT_ALSO_NEGOTIATES {object}s
 * @property INSUFFICIENT_STORAGE {object}
 * @property LOOP_DETECTED {object}
 * @property BANDWIDTH_sLIMIT_EXCEEDEDs {object}
 * @property NOT_EXTENDED {object}
 * @propertys NETWORK_AUTHENTICATION_REQUIRED {object}
...
var exports = module.exports = {
	...
	FORBIDDEN: {
		...
		' If the server does not wish to make this information available to the client, the status code 404 (Not' +
	...
	REQUEST_URI_TOO_LONG: {
		...
		' converted a POST request to a GET request with long query information, when the client has descended into a' +
		' URI black hole of redirection (e.g., a redirected URI prefix that points to a suffix of itself), or when' +
	...
	REQUESTED_RANGE_NOT_SATISFIABLE: {
		...
		HTTPStatusCode: 416,
		description: 'A server SHOULD return a response with this status code if a request included a' +
		' Range request-header field, and none of the range-specifier values in this field overlap the current extent' +
		' of the selected resource, and the request did not include an If-Range request-header field. (For' +
		' byte-ranges, this means that the first- byte-pos of all of the byte-range-spec values were greater than the' +
		' current length of the selected resource.) When this status code is returned for a byte-range request, the' +
		' response SHOULD include a Content-Range entity-header field specifying the current length of the selected' +
	...
	EXPECTATION_FAILED: {
		...
		' the server is a proxy, the server has unambiguous evidence that the request could not be met by the' +
		' next-hop server.'
	},
	I_AM_A_TEAPOT: {
		code: 'I_AM_A_TEAPOT',
		statusText: 'I\'m a teapot (RFC 2324)',
		HTTPStatusCode: 418,
	...
	ENHANCE_YOUR_CALM: {
		...
		' number\'s association with marijuana. Other services may wish to implement the 429 Too Many Requests' +
	...
	UNPROCESSABLE_ENTITY: {
		...
		' request entity (hence a 415(Unsupported Media Type) status code is inappropriate), and the syntax of the' +
		' request entity is correct (thus a 400 (Bad Request) status code is inappropriate) but was unable to process' +
		' the contained instructions. For example, this error condition may occur if an XML request body contains' +
	...
	RESERVED_FOR_WEBDAV: {
		code: 'RESERVED_FOR_WEBDAV',
		statusText: 'Reserved for WebDAV',
		HTTPStatusCode: 425,
	...
	UPGRADE_REQUIRED: {
		...
		' The 426 Upgrade Required status code allows a server to definitively state the precise protocol extensions' +
	...
	BLOCKED_BY_WINDOWS_PARENTAL_CONTROLS: {
		...
		statusText: 'Blocked by Windows Parental Controls (Microsoft)',
		HTTPStatusCode: 450,
	...
	HTTP_VERSION_NOT_SUPPORTED: {
		...
		HTTPStatusCode: 505,
		description: 'The server does not support, or refuses to support, the HTTP protocol version that was used in' +
		' the request message. The server is indicating that it is unable or unwilling to complete the request using' +
		' the same major version as the client, as described in section 3.1, other than with this error message. The' +
		' response SHOULD contain an entity describing why that version is not supported and what other protocols are' +
	...
	NETWORK_READ_TIMEOUT_ERROR: {
		code: 'NETWORK_READ_TIMEOUT_ERROR',
